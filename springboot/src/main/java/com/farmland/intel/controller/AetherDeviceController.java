...
public class AetherDeviceController {
    ...
    // 设备状态缓存（volatile 保证可见性，synchronized 保证复合操作原子性）
    private static final Object stateLock = new Object();
    private static volatile Double currentTemperature = 25.5;
    private static volatile Double currentHumidity = 60.0;
    private static volatile Integer currentLed = 0;
            ...
            try {
                ...
                if (Boolean.TRUE.equals(oneNetData.getOrDefault("success", false))) {
                    ...
                    synchronized (stateLock) {
                        ...
                        if (tempObj != null) currentTemperature = ((Number) tempObj).doubleValue();
                        if (humiObj != null) currentHumidity = ((Number) humiObj).doubleValue();
                        if (ledObj != null) currentLed = ((Number) ledObj).intValue();
                        if (onlineObj != null) deviceOnline = Boolean.TRUE.equals(onlineObj);
    ...
    public Result controlLed(@RequestBody Map<String, Object> params) {
        Result authResult = requireAdmin();
        if (authResult != null) {
            return authResult;
        }
    ...
    public Result getReadingsDetail(@RequestParam(defaultValue = "30") Integer days) {
        ...
        try {
            ...
            log.error("从数据库获取历史数据失败", e);
            return Result.error("500", "数据库查询失败，请稍后重试");
        }
    }
    ...
    private Result requireAdmin() {
        ...
        if (currentUser == null || !"ROLE_ADMIN".equals(currentUser.getRole())) {
            return Result.error(Constants.CODE_401, "无权限，仅管理员可控制设备");
        }
        return null;
    }
    ...
    public Result simulateData() {
        ...
        log.info("模拟数据生成成功: 温度={}℃, 湿度={}%", temp, humi);
