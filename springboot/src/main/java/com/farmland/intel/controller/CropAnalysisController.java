...
public class CropAnalysisController {
    ...
    public Result analyzeRipeness(@RequestParam("file") MultipartFile file) {
        try {
            ...
            ResponseEntity<Map> response = restTemplate.exchange(
                    PYTHON_API_URL + "/detect/ripeness",
                    HttpMethod.POST,
                    requestEntity,
                    Map.class
            );
    ...
    public Result getModels() {
        try {
            ResponseEntity<Map> response = restTemplate.getForEntity(PYTHON_API_URL + "/models", Map.class);
            Map responseBody = response.getBody();
            if (responseBody != null && "200".equals(responseBody.get("code"))) {
                return Result.success(responseBody.get("data"));
            }
    ...
    public Result healthCheck() {
        try {
            ResponseEntity<Map> response = restTemplate.getForEntity(PYTHON_API_URL + "/health", Map.class);
            Map responseBody = response.getBody();
            if (responseBody != null && "200".equals(responseBody.get("code"))) {
                return Result.success(responseBody.get("data"));
            }
