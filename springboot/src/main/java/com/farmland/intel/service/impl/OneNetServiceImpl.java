...
public class OneNetServiceImpl implements IOneNetService {
    ...
    public Map<String, Object> getDeviceData() {
        ...
        try {
            ...
            log.info("📡 OneNET响应状态: {}", response.getStatusCode());
            ...
            if (response.getStatusCode() == HttpStatus.OK && response.getBody() != null) {
                ...
                if (dataObj instanceof java.util.List) {
                    ...
                    if (dataList.size() >= 3) {
                        ...
                        log.info("✅ STM32数据解析成功 - 温度: {}℃, 湿度: {}%", temperature, humidity);
    ...
    public boolean controlLed(boolean led) {
        try {
            ...
            if (success) {
                log.info("✅ OneNET LED控制成功: {}", led ? "开启" : "关闭");
            } else {
                log.warn("⚠️ OneNET LED控制失败，状态码: {}", response.getStatusCode());
            }
            return success;
        } catch (Exception e) {
            log.error("❌ 控制LED失败", e);
    ...
    public boolean syncDataToDatabase() {
        try {
            ...
            if ((Boolean) data.getOrDefault("success", false)) {
                ...
                log.info("✅ [定时任务] STM32数据已同步到数据库: T={}℃, H={}%", reading.getTemperature(), reading.getHumidity());
                return true;
            } else {
                log.warn("⚠️ [定时任务] 获取设备数据失败，无法同步");
            }
        } catch (Exception e) {
...
public boolean controlBump(boolean bump) {
    try {
        ...
        log.info("🚿 正在控制水泵: {} -> {}", newProductId, bump ? "开启" : "关闭");
        ...
        if (success) {
            ...
            log.warn("⚠️ OneNET 水泵控制失败，状态码: {}", response.getStatusCode());
        }
        return success;
    } catch (Exception e) {
        log.error("❌ 控制水泵失败", e);
        ...
        if (response.getStatusCode() == HttpStatus.OK && response.getBody() != null) {
            ...
            if (dataObj instanceof java.util.List) {
                ...
                if (dataList.size() >= 4) {
                    ...
                    log.info("✅ 新设备数据解析成功 - 温度: {}℃, 湿度: {}%, 水泵: {}", temperature, humidity, bumpState ? "开" : "关");
