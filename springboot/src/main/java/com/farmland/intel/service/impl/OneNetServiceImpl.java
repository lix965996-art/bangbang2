...
public class OneNetServiceImpl implements IOneNetService {
    ...
    private String generateToken() {
        try {
            ...
            throw new RuntimeException("OneNET Token生成失败", e);
        }
    }
    ...
    private static boolean isLedIdentifier(String id) {
        if (id == null) return false;
        String s = id.trim().toLowerCase(Locale.ROOT);
        return "led".equals(s) || "light_switch".equals(s);
    }

    private static boolean isBumpIdentifier(String id) {
        if (id == null) return false;
        String s = id.trim().toLowerCase(Locale.ROOT);
        return "bump".equals(s) || "pump".equals(s) || "water_pump".equals(s);
    }
    ...
    public Map<String, Object> getDeviceData() {
        ...
        try {
            ...
            result.put("success", false);
            result.put("online", false);
            result.put("error", "从OneNET获取数据失败");
        }
    ...
    public boolean syncDataToDatabase() {
        try {
            ...
            if (Boolean.TRUE.equals(data.getOrDefault("success", false))) {
                SensorReading reading = new SensorReading();
                Object tempObj = data.get("temperature");
                Object humiObj = data.get("humidity");
                Object ledObj = data.get("led");
                if (tempObj == null || humiObj == null || ledObj == null) {
                    log.warn("[定时任务] 设备数据字段缺失: temp={}, humi={}, led={}", tempObj, humiObj, ledObj);
                    return false;
                }
                reading.setTemperature(((Number) tempObj).doubleValue());
                reading.setHumidity(((Number) humiObj).doubleValue());
                reading.setLed(((Number) ledObj).intValue());
...
public Map<String, Object> getNewDeviceData() {
    ...
    try {
        ...
        if (response.getStatusCode() == HttpStatus.OK && response.getBody() != null) {
            ...
            if (dataObj instanceof java.util.List) {
                ...
                Map<String, Object> tempItem = null, humiItem = null, bumpItem = null, ledItem = null;
                for (Map<String, Object> item : dataList) {
                    if (item == null) continue;
                    String id = item.get("identifier") != null ? item.get("identifier").toString() : null;
                    if (isTempIdentifier(id)) tempItem = item;
                    else if (isHumiIdentifier(id)) humiItem = item;
                    else if (isBumpIdentifier(id)) bumpItem = item;
                    else if (isLedIdentifier(id)) ledItem = item;
                }

                Object tempVal = tempItem != null ? tempItem.get("value") : null;
                Object humiVal = humiItem != null ? humiItem.get("value") : null;
                Object bumpVal = bumpItem != null ? bumpItem.get("value") : null;
                Object ledVal = ledItem != null ? ledItem.get("value") : null;
                if (tempVal == null || humiVal == null) {
                    log.warn("新设备数据字段缺失: temp={}, humi={}，请检查物模型 identifier", tempVal, humiVal);
                    result.put("online", online);
                    result.put("error", "OneNET 新设备未返回温湿度：检查物模型标识符是否为 temp/humi 或 temperature/humidity");
                    return result;
                }
                Double temperature = round(Double.parseDouble(tempVal.toString()), 1);
                Double humidity = round(Double.parseDouble(humiVal.toString()), 1);
                boolean bumpState = parseBooleanState(bumpVal);
                boolean ledState = parseBooleanState(ledVal);
    ...
    public boolean syncNewDeviceDataToDatabase() {
        try {
            ...
            if (Boolean.TRUE.equals(data.getOrDefault("success", false))) {
                Object tempObj = data.get("temperature");
                Object humiObj = data.get("humidity");
                Object bumpObj = data.get("bump");
                if (tempObj == null || humiObj == null) {
                    log.warn("[定时任务] 新设备数据字段缺失: temp={}, humi={}", tempObj, humiObj);
                    return false;
                }
                SensorReading reading = new SensorReading();
                reading.setTemperature(((Number) tempObj).doubleValue());
                ...
                boolean bumpState = Boolean.TRUE.equals(bumpObj);
                reading.setLed(bumpState ? 1 : 0);
    ...
    private boolean parseBooleanState(Object value) {
