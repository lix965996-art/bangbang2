...
当前项目已经整理为通过环境变量读取通义千问、高德地图和 OneNET 配置。
...
- `ONENET_NEW_PRODUCT_ID`
- `ONENET_NEW_DEVICE_NAME`

可选：

- `ONENET_VERSION`，默认 `2022-05-01`
...
- 地图前端与后端均改为读取环境变量
- OneNET 当前也改为从环境变量读取 `author-key`、`user-id`、`product-id`、`device-name`
- `ONENET_ENABLED` 可控制是否创建 OneNET 服务并执行同步任务
