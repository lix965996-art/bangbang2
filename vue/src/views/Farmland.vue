<template>
  <div class="farmland-manage">
      ...
      >
        ...
        <el-table-column label="资产操作" align="center">
          <template #default="scope">
            ...
            <el-button link class="del-btn" icon="el-icon-delete" @click="del(scope.row.id)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog custom-class="farmland-dialog" title="地块信息录入" v-model="dialogFormVisible" width="min(760px, 92vw)" :close-on-click-modal="false" append-to-body :lock-scroll="false">
        ...
        <el-form-item label="地理位置">
          ...
          <div v-if="form.coordinates && showPreviewMap" class="preview-map-container">
            <div class="preview-map-header">
              <div class="preview-map-title">
                <span class="preview-map-kicker">Spatial Boundary</span>
                <strong><i class="el-icon-map-location"></i> 地块空间预览</strong>
              </div>
              <div class="preview-map-actions">
                <el-button link icon="el-icon-map-location" @click="openLocationSelector" size="small">重新标记</el-button>
                <el-button link icon="el-icon-close" @click="collapsePreviewMap" size="small"></el-button>
              </div>
            </div>
            <div class="preview-map-meta">
              <div class="preview-meta-item">
                <span>中心坐标</span>
                <strong>{{ previewLocationText }}</strong>
              </div>
              <div class="preview-meta-item">
                <span>边界点</span>
                <strong>{{ previewCoordinateCount }} 个</strong>
              </div>
              <div class="preview-meta-item">
                <span>登记面积</span>
                <strong>{{ form.area || '--' }} 亩</strong>
              </div>
            </div>
            <div class="preview-map-shell">
              <div class="preview-map-badge">
...
export default {
  ...
  computed: {
    ...
    cropIconMap() {
      return {
        ...
        ai: assetUrl('aiye.png'),
        bamboo: assetUrl('zhuzi.png'),
        corn: assetUrl('yumi.png'),
        palm: assetUrl('zongshu.png'),
        tomato: assetUrl('xihongshi.png')
      }
    },
    previewCoordinateCount() {
      if (!this.form.coordinates) return 0
      try {
    ...
    previewLocationText() {
      const lng = Number(this.form.centerLng)
      const lat = Number(this.form.centerLat)
      if (!Number.isFinite(lng) || !Number.isFinite(lat)) return '待定位'
  ...
  methods: {
    restorePageScroll() {
      ...
      this.$nextTick(() => {
        const main = document.querySelector('.green-main')
        if (main) {
    ...
    del(id) {
      this.$confirm('此操作将永久删除该农田数据，是否继续？', '删除确认', {
    ...
    handleLocationConfirm(locationData) {
      ...
      if (locationData.district) {
        this.form.district = locationData.district
      ...
      try {
        ...
        this.$nextTick(() => {
          ...
          if (window.AMap.ToolBar) {
            this.previewMap.addControl(new window.AMap.ToolBar({
              position: { right: '14px', bottom: '54px' }
            }))
          }

          if (window.AMap.Scale) {
            this.previewMap.addControl(new window.AMap.Scale())
          }
          ...
          // 添加中心点标记
          this.previewCenterMarker = new window.AMap.Marker({
            position: [this.form.centerLng, this.form.centerLat],
            content: '<div class="preview-center-marker"><span></span></div>',
            offset: new window.AMap.Pixel(-13, -13)
          })
          this.previewMap.add(this.previewCenterMarker)
          
          // 自动缩放到合适视野
          this.previewMap.setFitView([this.previewPolygon])
          setTimeout(() => {
            if (this.previewMap) {
              this.previewMap.setZoom(Math.max(this.previewMap.getZoom() - 0.2, 13))
            }
          }, 80)
    ...
    extractDistrictFromAddress() {
      ...
      if (fullMatch) {
        this.form.district = fullMatch[1]
        return
      }
      ...
      if (cityDistrictMatch) {
        this.form.district = cityDistrictMatch[1] + cityDistrictMatch[2]
        return
      }
      ...
      for (const [district, fullDistrict] of Object.entries(zjjDistrictMapping)) {
        if (address.includes(district)) {
          this.form.district = fullDistrict
          return
        }
      }
      ...
      if (address.includes('张家界')) {
        this.form.district = '张家界市永定区'
        return
      }
      
      // 7. 包含"长沙"但没匹配到具体区县的，默认岳麓区
      if (address.includes('长沙')) {
        this.form.district = '长沙市岳麓区'
        return
      }
      
      this.form.district = '未知区县'  // 参考Django版本
    }
  }
}
</script>
...
:deep(.el-button--success.is-plain span ){
  color: #ffffff !important;
}
...
:deep(.custom-form .el-select__wrapper) {
  ...
  outline: 0 !important;
  box-shadow: inset 0 0 0 1px #e1e8e2 !important;
...
:deep(.custom-form .el-select__wrapper:hover) {
  ...
  box-shadow: inset 0 0 0 1px #b8d8bb !important;
}

:deep(.custom-form .el-input__wrapper.is-focus),
:deep(.custom-form .el-select__wrapper.is-focused) {
  background: #ffffff !important;
  box-shadow: inset 0 0 0 1px #4caf50, 0 0 0 3px rgba(76, 175, 80, 0.12) !important;
}

:deep(.custom-form .el-input__inner),
:deep(.custom-form .el-select__input) {
  border: 0 !important;
...
:deep(.custom-form .el-input__inner:focus),
:deep(.custom-form .el-select__input:focus) {
  border: 0 !important;
  box-shadow: none !important;
}

:deep(.custom-form .el-textarea__inner) {
  background: #f7faf8 !important;
  border: 0 !important;
  border-radius: 8px !important;
  outline: 0 !important;
...
:deep(.custom-form .el-textarea__inner:hover) {
  background: #ffffff !important;
  box-shadow: inset 0 0 0 1px #b8d8bb !important;
}

:deep(.custom-form .el-textarea__inner:focus) {
  background: #ffffff !important;
  border: 0 !important;
  box-shadow: inset 0 0 0 1px #4caf50, 0 0 0 3px rgba(76, 175, 80, 0.12) !important;
}

:deep(.custom-form .el-input-number__increase),
:deep(.custom-form .el-input-number__decrease) {
  background: transparent !important;
...
/* 预览地图样式 */
:deep(.farmland-dialog .el-dialog__body) {
  padding: 20px 28px 8px;
}

:deep(.farmland-dialog .el-dialog__header) {
  padding: 20px 28px 12px;
}

:deep(.farmland-dialog .el-dialog__footer) {
  padding: 10px 28px 20px;
}

.preview-map-container {
  margin-top: 16px;
  border: 1px solid rgba(41, 128, 72, 0.18);
  border-radius: 8px;
  overflow: hidden;
  background: #f8fbf8;
  box-shadow: 0 16px 34px rgba(24, 87, 45, 0.12);
}
...
.preview-map-header {
  ...
  gap: 16px;
  padding: 14px 18px;
  background: linear-gradient(135deg, #e9f7ea 0%, #ffffff 58%, #eef7f0 100%);
  border-bottom: 1px solid rgba(41, 128, 72, 0.12);
  color: #1f5f38;
}
...
.preview-map-meta {
  ...
  gap: 1px;
  background: rgba(41, 128, 72, 0.12);
}
...
.preview-map-badge {
  ...
  padding: 0 12px;
  border-radius: 6px;
  background: rgba(21, 111, 58, 0.9);
  color: #ffffff;
  font-size: 13px;
  font-weight: 700;
  box-shadow: 0 8px 18px rgba(21, 111, 58, 0.22);
}
...
:deep(.preview-center-marker) {
  width: 26px;
  height: 26px;
  border-radius: 50%;
  background: rgba(16, 142, 76, 0.18);
  border: 1px solid rgba(16, 142, 76, 0.3);
  display: flex;
  align-items: center;
  justify-content: center;
}

:deep(.preview-center-marker span) {
  width: 12px;
  height: 12px;
  border-radius: 50%;
  background: #168f4f;
  border: 3px solid #ffffff;
  box-shadow: 0 4px 12px rgba(22, 143, 79, 0.35);
}
...
:deep(.mobile-preview-dialog .el-dialog__header ){ display: none; }
:deep(.mobile-preview-dialog .el-dialog__body ){ padding: 0; }
