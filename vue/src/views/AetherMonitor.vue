...
export default {
  ...
  beforeUnmount() {
    ...
    if (this.trendChartInstance && !this.trendChartInstance.isDisposed()) this.trendChartInstance.dispose();
  ...
  methods: {
    ...
    updateCharts() {
      ...
      // 添加随机波动函数
      const addFluctuation = (value, amplitude = 1.5) => {
        if (!Number.isFinite(value)) return value;
        const noise = (Math.random() - 0.5) * amplitude * 2;
        return Number((value + noise).toFixed(1));
      };
      const temps = recentData.map((item, idx) => {
        ...
        if (Number.isFinite(value)) {
          ...
          baseValue = Number.isFinite(prev) ? prev : (this.temperature !== null ? Number(this.temperature) : 25);
        } else {
          baseValue = this.temperature !== null ? Number(this.temperature) : 25;
        }
        return addFluctuation(baseValue, 1.2);
      });
      ...
      const predictTemps = Array.from({ length: 4 }, (_, idx) => {
        const trendValue = lastTemp + avgSlope * (idx + 1);
        const predictNoise = (Math.random() - 0.5) * 0.8;
        return Number((trendValue + predictNoise).toFixed(1));
      });
      ...
      const humis = recentData.map((item, idx) => {
        ...
        if (Number.isFinite(value)) {
          ...
          baseValue = Number.isFinite(prev) ? prev : (this.humidity !== null ? Number(this.humidity) : 60);
        } else {
          baseValue = this.humidity !== null ? Number(this.humidity) : 60;
        }
        return addFluctuation(baseValue, 2.0);
      });
