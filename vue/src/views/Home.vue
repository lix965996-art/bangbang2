    ...
    <div class="bento-grid">
      ...
      <div class="card ai-card">
        ...
        <div class="card-header">
          ...
          <el-tag size="small" effect="dark" color="#8b5cf6" style="border:none">今日已生成</el-tag>
        </div>
        <div class="ai-body">
      ...
      <div class="card task-card">
        <div class="card-header-simple">
          ...
          <el-tag size="small" type="info">3 项</el-tag>
        </div>
        <div class="task-list">
          <div class="task-item" v-for="(task, i) in tasks" :key="i" :class="{ done: task.done }">
            ...
            <el-tag size="small" :type="task.tagType" effect="light">{{ task.tag }}</el-tag>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    async fetchSensorData() {
      ...
      } catch(e) { console.warn('获取传感器数据失败', e) }
    },
    async toggleDevice(type) {
      const newState = !this.iotState[type];
      ...
      try {
        ...
        this.iotState[type] = newState;
        this.$message.success(`指令下发成功：${newState ? '开启' : '关闭'} ${deviceMap[type]}`);
      } catch (e) {
        this.$message.error(`${deviceMap[type]}控制失败，请重试`);
      }
    },
    formatNumber(num) {
...
:deep(.farm-select .el-select__wrapper),
:deep(.farm-select .el-input__wrapper) {
  min-height: 44px;
  border: 0 !important;
  border-radius: 12px;
  background: linear-gradient(135deg, #f8fafc 0%, #f1f5f9 100%);
  box-shadow: inset 0 0 0 1px #e2e8f0, 0 2px 8px rgba(0, 0, 0, 0.04) !important;
  transition: background-color 0.3s ease, box-shadow 0.3s ease;
}
:deep(.farm-select .el-select__wrapper:hover),
:deep(.farm-select .el-input__wrapper:hover) {
  background: linear-gradient(135deg, #ffffff 0%, #f0fdf4 100%);
  box-shadow: inset 0 0 0 1px #10b981, 0 2px 8px rgba(16, 185, 129, 0.08) !important;
}
:deep(.farm-select .el-select__wrapper.is-focused),
:deep(.farm-select .el-input__wrapper.is-focus) {
  background: #ffffff;
  box-shadow: inset 0 0 0 1px #10b981, 0 0 0 3px rgba(16, 185, 129, 0.15) !important;
}
:deep(.farm-select .el-input__inner) {
  border: 0 !important;
