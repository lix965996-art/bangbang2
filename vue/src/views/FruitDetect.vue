...
export default {
  ...
  computed: {
    ...
    analysisServiceBadge() {
      ...
      if (this.streamActive) {
        return {
          ...
          label: '画面已接通 · 分析服务离线',
          tip: '当前画面已接通，但 AI 分析服务暂不可用，请联系管理员检查识别服务。'
        };
      }
      return {
        cls: 'offline',
    ...
    resultImageSrc() {
      ...
      if (this.analysisResult.result_image_url) return this.analysisResult.result_image_url;
      if (this.analysisResult.result_image) return this.analysisResult.result_image;
      if (this.analysisResult.disease_analysis && this.analysisResult.disease_analysis.result_image_url) return this.analysisResult.disease_analysis.result_image_url;
      ...
      if (this.analysisResult.ripeness_analysis && this.analysisResult.ripeness_analysis.result_image_url) return this.analysisResult.ripeness_analysis.result_image_url;
...
:deep(.custom-select .el-input__wrapper.is-focus ){
  box-shadow: inset 0 0 0 1px #739d89, 0 0 0 3px rgba(115, 157, 137, 0.1) !important;
}
...
:deep(.custom-select .el-input__inner:focus ){ border: 0 !important; box-shadow: none !important; }
