...
export const loadAmapSdk = async (options = {}) => {
  ...
  const key = options.key || import.meta.env.VUE_APP_AMAP_JS_KEY || mapConfig.amap.jsKey || ''
  const securityCode = options.forceNoSecurity
    ? ''
    : (options.securityCode || import.meta.env.VUE_APP_AMAP_SECURITY_CODE || mapConfig.amap.securityCode || '')
